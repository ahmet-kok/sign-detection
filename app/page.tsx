  ...
  const detectTrafficSigns = useCallback(async () => {
    ...
    try {
      ...
      if (result.detections.length > 0) {
        ...
        console.log(`🤖 Starting AI classification of detected regions with ${selectedModels.length} models...`);
        ...
        console.log("✅ Multi-model region-based detection and classification complete!");
      } else {
        // Step 2: Fallback - Classify entire image with multiple models
        console.log(`⚠️ No regions detected, trying full image classification with ${selectedModels.length} models...`);
        ...
        if (finalDetections.length > 0) {
          ...
          console.log("❌ No signs detected even with multi-model full image classification");
        }
      }
      ...
      if (finalDetections.length > 0) {
        ...
        // Crop the best detection (first one - highest confidence)
        ...
        setDetections([]);
        setError(`No traffic signs detected in the image (tried both region detection and full image classification with ${selectedModels.length} models)`);
      }
    } catch (err) {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-600">
            Upload an image to detect traffic signs with OpenCV.js and classify them using PyTorch AI
          </p>
          <p className="text-sm text-gray-500 mt-2">
            Combines computer vision region detection with deep learning classification (99.45% accuracy)
          </p>
        </div>
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex space-x-2">
                <button
                  ...
                  disabled={availableModels.length === 0 || selectedModels.length === availableModels.length}
            ...
            {isLoadingModels ? (
              ...
              <div>
                <p className="text-sm text-gray-600 mb-3">
                  Select models to use for classification. Multiple models will run in parallel for comparison.
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {availableModels.map((modelName) => (
                    <label key={modelName} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 cursor-pointer">
                      <input
                        type="checkbox"
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-gray-500">TensorFlow/Keras Model</div>
                      </div>
                    </label>
                  ))}
                </div>
                {selectedModels.length > 0 && (
                  <div className="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
                    <p className="text-sm text-blue-800">
                      <span className="font-medium">{selectedModels.length} model(s) selected:</span> {selectedModels.join(', ')}
                    </p>
                    <p className="text-xs text-blue-600 mt-1">
                      Each model will process the image independently and results will be compared
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="text-center py-4">
                <p className="text-gray-600">No models available. Please ensure your backend is running and models are loaded.</p>
                <button
        ...
        {processingSteps && (
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {/* Upscaled Step (only shown if upscaling was applied) */}
              {processingSteps &&
                ...
                <div key={index} className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                  ...
                  {detection.classification ? (
                    <div className="space-y-3">
                      <div className="bg-green-50 p-3 rounded-lg border border-green-200">
                        <div className="flex justify-between items-center mb-2">
                          <h4 className="font-medium text-green-800">🤖 Best AI Classification</h4>
                          <span className="bg-green-100 text-green-800 px-2 py-1 rounded-full text-sm font-medium">
                            {(detection.classification.ai_confidence * 100).toFixed(1)}% confident
                          </span>
                        </div>
                        ...
                        {detection.classification.top3_predictions && (
                          <div className="mt-3">
                            <h5 className="text-sm font-medium text-green-700 mb-2">Top 3 Predictions:</h5>
                            <div className="space-y-1">
                              {detection.classification.top3_predictions.map((pred, predIndex) => (
                                <div key={predIndex} className="flex justify-between items-center text-sm">
                                  <span className={predIndex === 0 ? "font-semibold" : ""}>
                      ...
                      {detection.multiModelResults && detection.multiModelResults.length > 1 && (
                        <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
                          <h4 className="font-medium text-blue-800 mb-3">
                            📊 All Model Results ({detection.multiModelResults.length} models)
                          </h4>
                          <div className="space-y-2">
                            {detection.multiModelResults
                              .sort((a, b) => b.confidence - a.confidence)
                              .map((result, resultIndex) => (
                              <div key={resultIndex} className="bg-white p-2 rounded border border-blue-200">
                                <div className="flex justify-between items-center">
                                  <div className="flex-1">
                                    ...
                                    <div className="text-xs text-gray-500">
                                      Input: {result.input_size} • Type: {result.model_type}
                                    </div>
                                  </div>
                                  <div className="text-right">
                                    ...
                                    {resultIndex === 0 && (
                                      <div className="text-xs text-blue-600 mt-1">Highest</div>
                                    )}
                                  </div>
                                </div>
                              </div>
                          ...
                          <div className="mt-2 text-xs text-blue-600">
                            🔄 All models processed the same region independently
                          </div>
                        </div>
                      )}
                      ...
                      {detection.multiModelResults && detection.multiModelResults.length > 1 && (
                        <div className="bg-purple-50 p-3 rounded-lg border border-purple-200">
                          <h4 className="font-medium text-purple-800 mb-2">📈 Model Agreement</h4>
                          <div className="text-sm text-purple-700">
                            {(() => {
                              const predictions = detection.multiModelResults.map(r => r.class_name);
                              const uniquePredictions = [...new Set(predictions)];
                              const agreement = (predictions.length - uniquePredictions.length + 1) / predictions.length;
                                ...
                                <div>
                                  <div className="flex justify-between items-center">
                                    ...
                                    <span className={`font-medium ${
                                      agreement > 0.7 ? 'text-green-600' : 
                                      agreement > 0.5 ? 'text-yellow-600' : 'text-red-600'
                                    }`}>
                    ...
                    <div className="bg-yellow-50 p-3 rounded-lg border border-yellow-200">
                      <p className="text-yellow-800 text-sm">
                        ⚠️ AI classification unavailable (backend not connected or no models selected)
                      </p>
                    </div>
                  )}
                  ...
                  <div className="mt-3 pt-3 border-t border-gray-200">
                    <p className="text-xs text-gray-600">
                      ...
                      <strong> Size:</strong> {detection.width}×{detection.height}px
                    </p>
                  </div>
                </div>
              ))}
